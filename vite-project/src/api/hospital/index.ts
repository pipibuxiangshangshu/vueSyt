...
import type { DoctorInfoData,UserResponseData,DoctorResponseData,HospitalWordData, WXLoginResponseData, HospitalDetail, DepartmentResponse, LoginData, UserLoginResponseData } from "./type";
//枚举请求地址
enum API {
...
//获取医院详情的接口
export const reqHospitalDatail = (hoscode: String) => request.get<any, HospitalDetail>(API.HOSPITALDETAIL_URL + hoscode)
//获取医院科室的接口
export const reqHospitalDepartment = (hoscode: string) => request.get<any, DepartmentResponse>(API.HOSPITALDEPARTMENT_URL + hoscode)
//获取验证码接口
export const reqCode = (phone: string) => request.get<any, any>(API.GETUSECODE_URL + phone)
//用户登录接口 
export const reqUserLogin = (data: LoginData) => request.post<any, UserLoginResponseData>(API.USERLOGIN_URL, data)

//获取威胁你扫码登陆生成二维码需要参数接口
