<template>
    <div class="register">
        ...
        <div class="top">
            ...
            <div class="level">
                <svg t="1719036919441" class="icon" viewBox="0 0 1024 1024" version="1.1"
                    xmlns="http://www.w3.org/2000/svg" p-id="4285" width="16" height="16">
                    <path
        ...
        <div class="department">
            <div class="leftNav">
                <ul>
                    <li @click="changeIndex(index)" :class="{ active: index == currentIndex }"
                        v-for="(department, index) in hospitalStore.DepartmentInfo" :key="department.depcode">{{
                        department.depname }}
                    </li>
                </ul>
            </div>
            <div class="rightDepartment">
                <!--用一个div代表:大科室与小科室-->
                <div  v-for="department in hospitalStore.DepartmentInfo" class="showDepartment">
                    <h4 class="cur">{{department.depname}}</h4>
                    <!--每一个大科室下的小科室-->
                    <ul>
...
const changeIndex = (index: number) => {
    ...
    //点击导航获取右侧科室（大科室的h4）
    let allH4  = document.querySelectorAll('.cur')
    //滚动到对应科室的位置
    allH4[currentIndex.value].scrollIntoView({
        behavior:'smooth'
    });
}
