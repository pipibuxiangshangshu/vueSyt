<template>
    <!-- 订单详情结构 -->
    <el-card class="box-card" style="max-width: 950px">
        <!--卡片头部的结构-->
        <template #header>
        ...
        <div class="top">
            ...
            <el-tag type="success">
                <div class="tag">
                    <svg t="1719401688137" class="icon" viewBox="0 0 1024 1024" version="1.1"
                        xmlns="http://www.w3.org/2000/svg" p-id="4220" width="16" height="16">
                        <path d="M858.7 258.8L442.4 675.1 165 397.6l-45 45 322.7 322.6L904 304z" fill="#1afa29"
                            p-id="4221">
                        </path>
                    </svg>
            ...
            <div class="left">
                ...
                <div class="button" v-if="orderInfo.orderStatus == 0 || orderInfo.orderStatus == 1">
                    <el-popconfirm title="确定要取消预约吗?" @confirm="cancel">
                        <template #reference>
                            <el-button style="border: 1px solid #ccc;width: 80px;">取消预约</el-button>
                        </template>
                    </el-popconfirm>
                    <el-button @click="openDialog" v-if="orderInfo.orderStatus == 0" type="primary"
                        style="background-color: #3E9EFD;width: 60px;">支付</el-button>
                </div>
            </div>
            <div class="right">
                <!--右侧卡片-->
                <el-card style="max-width: 600px">
                    ...
                    <p style="color: red;">2.【取号】就诊当天需在{{ orderInfo.fetchTime }}前前 在医院取号，未取号视为爽约，该号不退不换；</p>
    ...
    <!--对话框通过v-model控制显示与隐藏-->
    <div>
        <el-dialog @close="close" v-model="dialogTableVisible" title="微信支付" width="500">
            <!--支付需要使用的二维码图片-->
            <div class="qrcode">
                <img :src="imgUrl" alt="">
                <p>请使用微信扫一扫</p>
            ...
            <!--对话框底部插槽-->
            <template #footer>
                <el-button @click="closeDialog" type="primary" style="width: 70px;">关闭窗口</el-button>
            </template>
        </el-dialog>
    </div>
</template>
...
import { OrderResponseData,QrCode,PayResult } from '@/api/user/type';
...
//取消订单 订单状态三种orderStatus -1 取消预约 0 预约没支付  1支付成功
const cancel = async () => {
    try {
        //取消预约成功
